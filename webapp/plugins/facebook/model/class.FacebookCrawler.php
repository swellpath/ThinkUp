...
class FacebookCrawler {
    ...
    public function fetchActiveUsers($recreate_all_history = false) {
        ...
        $active_users_dao = DAOFactory::getDAO('ActiveUsersCountDAO');

        // Latest referral data we have captured
        $latest_captured = $active_users_dao->getLatest($this->instance->id);
        ...
        while ($fetch_prev_page) {
            $stream = FacebookGraphAPIAccessor::rawApiRequest($next_api_request, true);
            if (isset($stream->data) && is_array($stream->data) && sizeof($stream->data) > 0) {
                $this->logger->logInfo(sizeof($stream->data[0]->values) .
                " day(s) of active user data found on page ".$current_page_number, __METHOD__.','.__LINE__);

                foreach ($stream->data as $period_group) {
                    $period_length = $period_group->period;
                    foreach ($period_group->values as $period) {
                        // End times are midnight PST/PDT, e.g. 2011-10-17T07:00:00+0000
                        $date = date('Y-m-d', strtotime($period->end_time) - 1);
                        $active_users = $period->value;
                        $active_users_dao->upsert($this->instance->id, $date, $period_length, $active_users);
                    }
                }
                ...
                if (isset($stream->paging->previous) && $first_post && $starts > $first_post &&
                ($starts > $latest_captured || $recreate_all_history)) {
                ...
                $this->logger->logInfo("No Facebook active user data found for ID $id", __METHOD__.','.__LINE__);
                $fetch_prev_page = false;
            }
            if (time() > $fetch_stop_time) {
                $fetch_prev_page = false;
